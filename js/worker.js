...
// gemm-precision: int8shiftAlphaAll
    ...
    try {
      ...
      postMessage(['log', "Models installed."]);
    } catch (error) {
      postMessage(['log', "Error installing models."]);
      log(`Exception ${error}`);
    }
  }
  ...
  const translate = (paragraphs) => {
    ...
    try {
      ...
      postMessage(['log', "Error translating."]);
      log(`Exception ${error}`);
    }
  }
...
onmessage = async function(e) {
    ...
    if (e.data[0] === 'load') {
        ...
        importScripts("modelRegistry.js");
        postMessage(['modelRegistry', modelRegistry]);
